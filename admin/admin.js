...
function populateProductCategorySelect(selectedCategory='', selectedSubcategory=''){
  ...
  catSelect.innerHTML='<option value="">Wybierz kategorię</option>'+categories.map(c=>`<option value="${c.name}">${c.name}</option>`).join('');
  catSelect.value=selectedCategory || '';
  const current=categories.find(c=>String(c.name).toLowerCase()===String(catSelect.value).toLowerCase());
  const subs=current&&Array.isArray(current.subcategories)?current.subcategories:[];
  subSelect.innerHTML='<option value="">Wybierz podkategorię</option>'+subs.map(s=>`<option value="${s.name}">${s.name}</option>`).join('');
...
$('product-upload').addEventListener('change',e=>fileToDataUrl(e.target,url=>{ $('product-img').value=url; const prev=$('product-preview'); if(prev) prev.src=url; }));
$('category-upload').addEventListener('change',e=>fileToDataUrl(e.target,url=>{ $('category-img').value=url; const prev=$('category-preview'); if(prev) prev.src=url; }));
...
function clearProduct(){['product-id','product-name','product-price','product-badge','product-img','product-features'].forEach(id=>$(id).value='');populateProductCategorySelect('','');$('product-visible').checked=true;$('product-promo').checked=true;$('product-discount').checked=true}$('clear-product').onclick=clearProduct;
function editProduct(id){const p=data.products.find(x=>x.id===id);if(!p)return;$('product-id').value=p.id;$('product-name').value=p.name;$('product-price').value=p.price;populateProductCategorySelect(p.category||'',p.subcategory||'');$('product-badge').value=p.badge||'';$('product-img').value=p.img||'';$('product-features').value=(p.features||[]).join('\n');$('product-visible').checked=p.visible!==false;$('product-promo').checked=p.promo!==false;$('product-discount').checked=p.randomDiscount!==false;window.scrollTo({top:0,behavior:'smooth'})}
...
$('category-form').addEventListener('submit',e=>{e.preventDefault();const id=$('category-id').value||('c'+Date.now());const c={id,name:$('category-name').value.trim(),description:$('category-description').value.trim(),img:$('category-img').value.trim(),subcategories:$('category-subcategories').value.split('\n').map(line=>{const [name,description='']=line.split('|').map(x=>x.trim());return name?{name,description}:null}).filter(Boolean)};const i=data.categories.findIndex(x=>x.id===id);if(i>=0)data.categories[i]=c;else data.categories.push(c);clearCategory();renderAll();saveData()});
function renderAll(){populateProductCategorySelect($('product-category')?$('product-category').value:'',$('product-subcategory')?$('product-subcategory').value:'');renderProducts();renderCategories()}
