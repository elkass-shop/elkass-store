...
q('previewBtn').onclick=render;q('printOnlyBtn').onclick=()=>{render();openPremiumPrint()};q('saveBtn').onclick=()=>{const p=data(); const arr=JSON.parse(localStorage.getItem('elkassBotPremiumProducts')||'[]'); arr.unshift({...p,id:'bot-'+Date.now(),status:'do akceptacji'}); localStorage.setItem('elkassBotPremiumProducts',JSON.stringify(arr)); alert('Zapisano do akceptacji.');};q('exportBtn').onclick=()=>{const blob=new Blob([JSON.stringify(data(),null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='elkass-produkt-premium.json'; a.click();};
render();
