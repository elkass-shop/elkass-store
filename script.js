...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const defaultCategories = [
    {id:'c1', name:'RTV', description:'Telewizory, soundbary, kino domowe', img:'assets/categories/rtv.svg', subcategories:[{name:'Telewizory', description:'Smart TV, QLED, OLED i 4K'}, {name:'Soundbary', description:'Lepszy dźwięk do telewizora'}, {name:'Audio', description:'Głośniki, wieże i kino domowe'}]},
    {id:'c2', name:'AGD', description:'Lodówki, pralki, zmywarki', img:'assets/categories/agd.svg', subcategories:[{name:'Lodówki', description:'No Frost i duże pojemności'}, {name:'Pralki', description:'Slim, standard i pralko-suszarki'}, {name:'Zmywarki', description:'45 cm, 60 cm i do zabudowy'}]},
    {id:'c3', name:'Komputery', description:'Laptopy, monitory, akcesoria', img:'assets/categories/komputery-telefony.svg', subcategories:[{name:'Laptopy', description:'Do pracy, nauki i domu'}, {name:'Monitory', description:'Biuro, gaming i multimedia'}, {name:'Akcesoria', description:'Myszy, klawiatury, drukarki'}]},
    {id:'c4', name:'Telefony', description:'Smartfony, tablety, akcesoria', img:'assets/categories/komputery-telefony.svg', subcategories:[{name:'Smartfony', description:'Android, 5G i duże baterie'}, {name:'Tablety', description:'Do szkoły, pracy i rozrywki'}, {name:'Akcesoria GSM', description:'Ładowarki, szkła i etui'}]},
    {id:'c5', name:'Audio', description:'Słuchawki, głośniki, soundbary', img:'assets/categories/rtv.svg', subcategories:[{name:'Słuchawki', description:'Bluetooth, ANC i sportowe'}, {name:'Głośniki', description:'Przenośne i domowe'}, {name:'Soundbary', description:'Do telewizora i kina domowego'}]},
    {id:'c6', name:'Serwis', description:'Pomoc, konfiguracja, doradztwo', img:'assets/categories/serwis.svg', subcategories:[{name:'Wsparcie', description:'Pomoc po zakupie'}, {name:'Konfiguracja', description:'Ustawienie sprzętu i aplikacji'}, {name:'Doradztwo', description:'Dobór sprzętu do potrzeb'}]}
  ];
  ...
  function renderProductDetailPage(){
    ...
    function printPremiumProductCard(){
      const printSpecs = specs.slice(0, 12);
      const printFeatures = features.slice(0, 6);
      const printWindow = window.open('', '_blank', 'width=980,height=720');
      if(!printWindow){ window.print(); return; }
      const today = new Date().toLocaleDateString('pl-PL');
      const oldPriceHtml = discount ? `<span class="old-price">${escapeHtml(formatPrice(oldPrice))}</span>` : '';
      const discountHtml = discount ? `<span class="discount-pill">-${discount}%</span>` : '';
      const specsHtml = printSpecs.length ? printSpecs.map(s => `<div class="spec"><strong>${escapeHtml(s.name)}</strong><span>${escapeHtml(s.value)}</span></div>`).join('') : '<div class="spec"><strong>Doradztwo</strong><span>Dobierzemy sprzęt do potrzeb klienta</span></div>';
      const featuresHtml = printFeatures.length ? printFeatures.map(f => `<li>${escapeHtml(f)}</li>`).join('') : '<li>Produkt dostępny w ofercie sklepu ELKASS</li><li>Możliwość zakupu na raty</li>';
      const thumbsHtml = gallery.slice(0,4).map((img,i)=>`<div class="thumb ${i===0?'main':''}"><img src="${escapeHtml(img)}"><span>${i===0?'GŁÓWNE':'FOTO '+(i+1)}</span></div>`).join('');
      printWindow.document.open();
      printWindow.document.write(`<!doctype html><html lang="pl"><head><meta charset="utf-8"><title>Karta produktu - ${escapeHtml(product.name)}</title><style>
        @page{size:A4;margin:10mm}*{box-sizing:border-box}body{margin:0;font-family:Arial,Helvetica,sans-serif;color:#111;background:#fff}.sheet{width:190mm;min-height:277mm;margin:0 auto;padding:0;display:flex;flex-direction:column;gap:7mm}.top{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #111;padding-bottom:5mm}.brand{font-size:28px;font-weight:900;letter-spacing:-1px}.brand span{color:#e30613}.brand small{display:block;font-size:10px;letter-spacing:2px;margin-top:2px}.meta{text-align:right;font-size:10px;color:#555;line-height:1.5}.hero{display:grid;grid-template-columns:72mm 1fr;gap:8mm;align-items:start}.photo{height:88mm;border:1px solid #e5e7eb;border-radius:8mm;background:#f8fafc;display:flex;align-items:center;justify-content:center;padding:5mm}.photo img{max-width:100%;max-height:100%;object-fit:contain}.info h1{font-size:22px;line-height:1.08;margin:0 0 4mm;font-weight:900;letter-spacing:-.4px}.tags{display:flex;gap:2mm;flex-wrap:wrap;margin-bottom:4mm}.tag{border:1px solid #dbeafe;background:#f8fbff;border-radius:999px;padding:2mm 3mm;font-size:9px;font-weight:800}.tag.red{background:#e30613;color:#fff;border-color:#e30613}.availability{display:inline-block;background:#ecfdf5;color:#087345;border-radius:999px;padding:2mm 3mm;font-size:10px;font-weight:900;margin:1mm 0 4mm}.price-row{display:flex;align-items:center;gap:3mm;margin:2mm 0 4mm}.price{font-size:29px;font-weight:900;color:#e30613}.old-price{text-decoration:line-through;color:#777;font-size:13px;font-weight:700}.discount-pill{background:#111;color:#fff;border-radius:999px;padding:2mm 3mm;font-size:10px;font-weight:900}.lead{font-size:11px;line-height:1.45;color:#344054;margin:0}.offers{display:grid;grid-template-columns:repeat(2,1fr);gap:3mm}.offer{border:1px solid #dbeafe;border-left:3px solid #0b63ce;border-radius:4mm;padding:3mm;font-weight:800;font-size:10px}.features{border:1px solid #e5e7eb;border-radius:5mm;padding:4mm}.features h2,.specs h2{font-size:14px;margin:0 0 3mm}.features ul{margin:0;padding-left:5mm;columns:2;column-gap:7mm}.features li{font-size:10px;line-height:1.45;margin:0 0 1.5mm;break-inside:avoid}.specs{border:1px solid #e5e7eb;border-radius:5mm;padding:4mm}.spec-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:2.5mm}.spec{border:1px solid #edf0f5;background:#fafafa;border-radius:3mm;padding:2.5mm;min-height:15mm}.spec strong{display:block;font-size:9px;margin-bottom:1mm}.spec span{font-size:9.5px;color:#344054;line-height:1.35}.thumbs{display:grid;grid-template-columns:repeat(4,1fr);gap:2.5mm}.thumb{height:26mm;border:1px solid #e5e7eb;border-radius:3mm;display:flex;align-items:center;justify-content:center;position:relative;padding:2mm}.thumb img{max-width:100%;max-height:100%;object-fit:contain}.thumb span{position:absolute;left:2mm;top:2mm;background:#111;color:#fff;border-radius:999px;font-size:6.5px;padding:1mm 1.7mm;font-weight:800}.thumb.main span{background:#e30613}.footer{margin-top:auto;border-top:2px solid #111;padding-top:4mm;display:grid;grid-template-columns:1fr 1fr 1fr;gap:4mm;font-size:10px}.footer strong{display:block;margin-bottom:1mm}.note{font-size:8px;color:#666;margin-top:2mm}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.sheet{page-break-after:avoid}.no-print{display:none!important}}</style></head><body><div class="sheet">
        <header class="top"><div class="brand"><span>ELKASS</span> OLESNO<small>RTV / AGD / MULTIMEDIA / SERWIS</small></div><div class="meta">Karta produktu<br>${escapeHtml(today)}<br>${escapeHtml(product.category || 'Oferta')} / ${escapeHtml(product.subcategory || 'Produkt')}</div></header>
        <section class="hero"><div class="photo"><img src="${escapeHtml(mainImage)}" alt="${escapeHtml(product.name)}"></div><div class="info"><div class="tags"><span class="tag red">${escapeHtml(promotionLabel(product, product.badge || 'OFERTA'))}</span><span class="tag">${escapeHtml(product.category || 'Oferta')}</span><span class="tag">${escapeHtml(product.subcategory || 'Produkt')}</span></div><h1>${escapeHtml(product.name)}</h1><div class="availability">● ${escapeHtml(productAvailability(product))}</div><div class="price-row"><span class="price">${escapeHtml(formatPrice(price))}</span>${oldPriceHtml}${discountHtml}</div><p class="lead">Zapytaj o aktualną dostępność, odbiór w sklepie lub możliwość dostawy. Nasi doradcy pomogą dobrać najlepszy sprzęt do Twoich potrzeb.</p></div></section>
        <section class="offers"><div class="offer">💳 Raty 0% i wygodne finansowanie</div><div class="offer">🚚 Odbiór osobisty w Olesnie</div><div class="offer">🛠 Fachowe doradztwo ELKASS</div><div class="offer">📞 Kontakt: 34 358 24 42</div></section>
        <section class="features"><h2>Najważniejsze cechy</h2><ul>${featuresHtml}</ul></section>
        <section class="specs"><h2>Parametry produktu</h2><div class="spec-grid">${specsHtml}</div></section>
        ${gallery.length > 1 ? `<section class="thumbs">${thumbsHtml}</section>` : ''}
        <footer class="footer"><div><strong>Salon ELKASS</strong>Armii Krajowej 5<br>46-300 Olesno</div><div><strong>Kontakt</strong>34 358 24 42<br>elkass@wp.pl</div><div><strong>Godziny otwarcia</strong>Pon-Pt 9:00–17:00<br>Sobota 9:00–13:00</div></footer><div class="note">Oferta informacyjna. Dostępność i cena mogą ulec zmianie. Potwierdź szczegóły w sklepie ELKASS Olesno.</div>
    ...
    if(printBtn) printBtn.addEventListener('click', printPremiumProductCard);
